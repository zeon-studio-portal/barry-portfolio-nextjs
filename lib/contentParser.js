...
export const getSinglePage = (folder) => {
  ...
  const sanitizeFiles = filesPath.filter((file) => file.includes(".mdx") || file.includes(".md"));
  ...
  const publishedPages = singlePages.filter((page) => !page.frontmatter.draft && !page.frontmatter.layout && page);
